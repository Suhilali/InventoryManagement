$(document).ready(function(){
	...
	$("#register_form").on("submit",function(){
		...
		if ((pass1.val() == pass2.val()) && status == true) {
			$(".overlay").show();
			$.ajax({
				url : "http://localhost/inv_project/includes/process.php",
				...
				success : function(data){
					if (data == "EMAIL_ALREADY_EXISTS") {
						...
						$(".overlay").hide();
						window.location.href = encodeURI("http://localhost/inv_project"+"/index.php?msg=You are registered Now you can login");
					}
				}
			})
		}else{
			...
			$.ajax({
				...
				success : function(data){
					if (data == "NOT_REGISTERD") {
						...
						$(".overlay").hide();
						console.log(data);
						window.location.href = "http://localhost/inv_project"+"/dashboard.php";
					}
				}
			})
		}
